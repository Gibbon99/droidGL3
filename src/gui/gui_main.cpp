...
#include "hdr/gui/gui_renderTransfer.h"
          ...
          switch (tempEventData.eventAction)
            {
              ...
              case USER_EVENT_GAME_TIMER:
                switch (tempEventData.data1)
                  {
                    case USER_EVENT_GUI_MOUSE_TIMER: io_mouseTimerState (tempEventData.data2);
                    break;

                    case USER_EVENT_GUI_ANIMATE_TIMER: gui_timerFocusAnimation (tempEventData.data2);
                    break;
...
//-----------------------------------------------------------------------------
{
  ...
  switch (guiObjectType)
    {
      case GUI_OBJECT_BUTTON: tmpGuiButton.attributes.objectID = objectID;
      ...
      case GUI_OBJECT_CHECKBOX: tmpGuiCheckBox.attributes.objectID = objectID;
      ...
      case GUI_OBJECT_TEXTBOX: tmpGuiTextBox.attributes.objectID = objectID;
      ...
      case GUI_OBJECT_LABEL: tmpLabel.objectID = objectID;
...
void
gui_hostSetObjectPosition (int guiObjectType, const string &objectID, int coordType, int startX, int startY, int width, int height)
//-----------------------------------------------------------------------------
{
#ifdef DEBUG_GUI_SETUP
  ...
  switch (guiObjectType)
    {
      case GUI_OBJECT_BUTTON: guiButtons[objectIndex].attributes.coordType = coordType;
      if (GUI_COORD_TYPE_PERCENT == guiButtons[objectIndex].attributes.coordType)
        {
          ...
          guiButtons[objectIndex].attributes.boundingBox.x = (int) ((winWidth * ((float) startX / 100.0f))
                                                                    + (guiButtons[objectIndex].attributes.boundingBox.w
                                                                       / 2));
          guiButtons[objectIndex].attributes.boundingBox.y = (int) ((winHeight * ((float) startY / 100.0f))
                                                                    + (guiButtons[objectIndex].attributes.boundingBox.h
                                                                       / 2));

          guiButtons[objectIndex].attributes.mouseHitBox.x =
              guiButtons[objectIndex].attributes.boundingBox.x - guiButtons[objectIndex].attributes.boundingBox.w;
      ...
      case GUI_OBJECT_CHECKBOX: guiCheckBoxes[objectIndex].attributes.coordType = coordType;
      if (GUI_COORD_TYPE_PERCENT == guiCheckBoxes[objectIndex].attributes.coordType)
        {
          ...
          guiCheckBoxes[objectIndex].attributes.boundingBox.x = (int) ((winWidth * ((float) startX
                                                                                    / 100.0f))); // + (guiCheckBoxes[objectIndex].attributes.boundingBox.w / 2));
          guiCheckBoxes[objectIndex].attributes.boundingBox.y = (int) ((winHeight * ((float) startY
                                                                                     / 100.0f))); // + (guiCheckBoxes[objectIndex].attributes.boundingBox.h / 2));
      ...
      case GUI_OBJECT_TEXTBOX: guiTextBoxes[objectIndex].attributes.coordType = coordType;
      if (GUI_COORD_TYPE_PERCENT == guiTextBoxes[objectIndex].attributes.coordType)
        {
          ...
          guiTextBoxes[objectIndex].attributes.boundingBox.x = (int) ((winWidth * ((float) startX
                                                                                   / 100.0f))); // + (guiCheckBoxes[objectIndex].attributes.boundingBox.w / 2));
          guiTextBoxes[objectIndex].attributes.boundingBox.y = (int) ((winHeight * ((float) startY
                                                                                    / 100.0f))); // + (guiCheckBoxes[objectIndex].attributes.boundingBox.h / 2));
      ...
      case GUI_OBJECT_LABEL: guiLabels[objectIndex].coordType = coordType;
      if (GUI_COORD_TYPE_PERCENT == guiLabels[objectIndex].coordType)
        {
          ...
          guiLabels[objectIndex].boundingBox.x = (int) ((winWidth * ((float) startX
                                                                     / 100.0f))); // + (guiCheckBoxes[objectIndex].attributes.boundingBox.w / 2));
          guiLabels[objectIndex].boundingBox.y = (int) ((winHeight * ((float) startY
                                                                      / 100.0f))); // + (guiCheckBoxes[objectIndex].attributes.boundingBox.h / 2));
        }
      else
        {
...
void
gui_setObjectColorByIndex (int guiObjectType, int objectIndex, int whichColor, Uint8 red, Uint8 green, Uint8 blue, Uint8 alpha)
//-----------------------------------------------------------------------------
{
  switch (guiObjectType)
    {
      ...
      case GUI_OBJECT_BUTTON:
        {
          ...
          switch (whichColor)
            {
              case GUI_ACTIVE_COL: guiButtons[objectIndex].attributes.hasFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_INACTIVE_COL: guiButtons[objectIndex].attributes.noFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_ACTIVE_CORNER_COL: guiButtons[objectIndex].cornerFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_INACTIVE_CORNER_COL: guiButtons[objectIndex].cornerNoFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_ACTIVE_LABEL_COL: guiButtons[objectIndex].attributes.labelHasFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_INACTIVE_LABEL_COL: guiButtons[objectIndex].attributes.labelNoFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;
      ...
      case GUI_OBJECT_CHECKBOX:
        {
          ...
          switch (whichColor)
            {
              case GUI_ACTIVE_COL: guiCheckBoxes[objectIndex].attributes.hasFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_INACTIVE_COL: guiCheckBoxes[objectIndex].attributes.noFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_ACTIVE_CORNER_COL: guiCheckBoxes[objectIndex].cornerFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_INACTIVE_CORNER_COL: guiCheckBoxes[objectIndex].cornerNoFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_ACTIVE_LABEL_COL: guiCheckBoxes[objectIndex].attributes.labelHasFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_INACTIVE_LABEL_COL: guiCheckBoxes[objectIndex].attributes.labelNoFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;
      ...
      case GUI_OBJECT_TEXTBOX:
        {
          ...
          switch (whichColor)
            {
              case GUI_ACTIVE_COL: guiTextBoxes[objectIndex].attributes.hasFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_INACTIVE_COL: guiTextBoxes[objectIndex].attributes.noFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_ACTIVE_CORNER_COL: guiTextBoxes[objectIndex].cornerFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_INACTIVE_CORNER_COL: guiTextBoxes[objectIndex].cornerNoFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_ACTIVE_LABEL_COL: guiTextBoxes[objectIndex].attributes.labelHasFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_INACTIVE_LABEL_COL: guiTextBoxes[objectIndex].attributes.labelNoFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;
      ...
      case GUI_OBJECT_LABEL:
        {
          ...
          printf ("Label color set to [ %i %i %i ]\n", red, green, blue);
          ...
          switch (whichColor)
            {
              ...
              printf ("Label color now  [ %i %i %i ]\n", guiLabels[objectIndex].hasFocusColor.r, guiLabels[objectIndex].hasFocusColor.g, guiLabels[objectIndex].hasFocusColor.b);

              break;

              case GUI_INACTIVE_COL:guiLabels[objectIndex].noFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;

              case GUI_ACTIVE_LABEL_COL:
                guiLabels[objectIndex].labelHasFocusColor = gui_mapRGBA (red, green, blue, alpha);

              printf ("Label color now  [ %i %i %i ]\n", guiLabels[objectIndex].labelHasFocusColor.r, guiLabels[objectIndex].labelHasFocusColor.g, guiLabels[objectIndex].labelHasFocusColor.b);


              break;

              case GUI_INACTIVE_LABEL_COL:guiLabels[objectIndex].labelNoFocusColor = gui_mapRGBA (red, green, blue, alpha);
              break;
...
void
gui_hostSetObjectColor (int guiObjectType, string objectID, int whichColor, Uint8 red, Uint8 green, Uint8 blue, Uint8 alpha)
//-----------------------------------------------------------------------------
{
#ifdef DEBUG_GUI_SETUP
  ...
  switch (guiObjectType)
    {
      ...
      case GUI_OBJECT_BUTTON: guiButtons[objectIndex].attributes.action = clickFunction;
      break;

      case GUI_OBJECT_CHECKBOX: guiCheckBoxes[objectIndex].attributes.action = clickFunction;
      break;

      case GUI_OBJECT_TEXTBOX: guiTextBoxes[objectIndex].attributes.action = clickFunction;
      break;
...
//--------------------------------------------------------------------------
{
  switch (errorCode)
    {
      case LEVEL_LOAD_ERROR_NOT_FOUND: con_print (CON_ERROR, true, "Could not find TTF file [ %s ]", fileName.c_str ());
      break;

      case LEVEL_LOAD_MALLOC_ERROR: con_print (CON_ERROR, true, "Memory loading error for [ %s ]", fileName.c_str ());
      break;

      case USER_EVENT_TTF_BAD_LOAD: con_print (CON_ERROR, true, "Error loading font [ %s ]", fileName.c_str ());
      break;

      case LEVEL_LOAD_ERROR_FILESYSTEM: con_print (CON_ERROR, true, "File system is not reading to load font [ %s ]", fileName.c_str ());
      break;

      case USER_EVENT_TTF_INIT_FAIL: con_print (CON_ERROR, true, "Unable to start TTF system [ %s ]", fileName.c_str ());
      break;

      case LEVEL_LOAD_MEMORY_ERROR: con_print (CON_ERROR, true, "Could not load file into memory [ %s ]", fileName.c_str ());
      break;
...
bool
gui_renderText (string fontIndexName, glm::vec2 position, glm::vec3 color, SDL_Surface *destSurface, std::string textString, ...)
//--------------------------------------------------------------------------
{
  unsigned int fontIndex;
  ...
  switch (moveDirection)
    {
      ...
      case FOCUS_PREVIOUS: indexCount = 1;
...
//-----------------------------------------------------------------------------
{
  ...
  if (eventAction == MY_INPUT_ACTION_RELEASE)
      ...
      switch (eventType)
        {
          ...
          case MY_INPUT_DOWN: evt_sendEvent (USER_EVENT_AUDIO, AUDIO_PLAY_SAMPLE, SND_KEYPRESS_GOOD, 0, 0, glm::vec2 (), glm::vec2 (), "");
          gui_handleFocusMove (FOCUS_NEXT, false, eventSource);
          break;

          case MY_INPUT_ACTION: evt_sendEvent (USER_EVENT_AUDIO, AUDIO_PLAY_SAMPLE, SND_KEYPRESS_GOOD, 0, 0, glm::vec2 (), glm::vec2 (), "");
          gui_handleFocusMove (-1, true, eventSource);  //eventSource is TRUE if event is from mouse
          break;

          default:
            if (GUI_OBJECT_TEXTBOX
                == guiScreens[currentGUIScreen].objectType[guiScreens[currentGUIScreen].selectedObject])
              {
  ...
  else
    {
      switch (eventType)
        {
          case MY_INPUT_CONSOLE: sys_changeMode (MODE_CONSOLE);
          break;
...
//-----------------------------------------------------------------------------
{
  ...
  for (auto it = guiScreens[currentGUIScreen].objectIDIndex.begin ();
       it != guiScreens[currentGUIScreen].objectIDIndex.end (); ++it)
    {
      switch (guiScreens[currentGUIScreen].objectType[indexCount])
        {
          case GUI_OBJECT_BUTTON:
