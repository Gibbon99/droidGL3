...
cpVect bul_getStartingPosition ( cpVect currentPos, int bulletType )
//-----------------------------------------------------------------------------
{
	cpVect bulletStart;
...
//-----------------------------------------------------------------------------
{
	...
	if ( -1 == sourceDroid )
	{
		tempBullet.travelDirection = bul_getPlayerBulletDirection ();
	...
	if ( BULLET_TYPE_DISRUPTER != type ) // if bullet is not a disrupter
	{
		...
		tempBullet.travelDirection = cpvnormalize ( tempBullet.travelDirection );
		tempBullet.worldPos = sourcePos;
		...
		tempBullet.worldPos = bul_getStartingPosition ( sourcePos, type );
		//
		...
		cpBodySetPosition ( tempBullet.bulletPhysicsObject.body, sourcePos );
		//
...
for (int i = 0; i != levelInfo.at ( levelName ).bullet.size(); i++)
	...
	levelInfo.at ( levelName ).bullet.push_back ( tempBullet );
	int bulletIndex = sys_pack4Bytes ((char) lvl_getDeckNumber ( levelName ), static_cast<char>(levelInfo.at ( levelName ).bullet.size()), 0, 0 );
		...
		if ( levelInfo.at ( levelName ).bullet[i].type != BULLET_TYPE_DISRUPTER )
		{
			if ( levelInfo.at (levelName ).bullet[i].isAlive)
			{
				...
				gl_renderSprite ( levelInfo.at ( levelName ).bullet[i].bitmapName, glm::vec2{
						...
						levelInfo.at ( levelName ).bullet[i].worldPos.y}, levelInfo.at ( levelName ).bullet[i].currentAnimFrame, glm::vec3{
						-1, 0, 0} );
			}
		}
	}
