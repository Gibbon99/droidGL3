...
//-----------------------------------------------------------------------------------------------------
{
  switch (errorCode)
    {
      case LEVEL_LOAD_ERROR_NOT_FOUND: con_print (CON_ERROR, true, "Could not find level file [ %s ]", fileName.c_str ());
      lvl_setLevelError (fileName);
      break;

      case LEVEL_LOAD_ERROR_FILESYSTEM: con_print (CON_ERROR, true, "Could not load level file from memory [ %s ]", fileName.c_str ());
      lvl_setLevelError (fileName);
      break;

      case LEVEL_LOAD_MEMORY_ERROR: con_print (CON_ERROR, true, "Memory error loading level [ %s ]", fileName.c_str ());
      lvl_setLevelError (fileName);
      break;

      case LEVEL_LOAD_MALLOC_ERROR: con_print (CON_ERROR, true, "MALLOC error loading level [ %s ]", fileName.c_str ());
      lvl_setLevelError (fileName);
      break;

      case SIDEVIEW_FILE_NOT_FOUND: con_print (CON_ERROR, true, "Could not find sideview data file [ %s ]", fileName.c_str ());
      break;

      case SIDEVIEW_MALLOC_ERROR: con_print (CON_ERROR, true, "Memory error loading datafile [ %s ]", fileName.c_str ());
      break;

      case SIDEVIEW_LOAD_MEMORY_ERROR: con_print (CON_ERROR, true, "Could not load datafile into memory [ %s ]", fileName.c_str ());
      break;

      case SIDEVIEW_LOAD_ERROR_FILESYSTEM: con_print (CON_ERROR, true, "Error reading from file [ %s ]", fileName.c_str ());
      break;
