...
//-----------------------------------------------------------------------------------------------------
{
	...
	if ( nullptr ==
	     SOIL_load_image_from_memory ((const unsigned char *) textureMemory[(size_t) textureMemoryIndex].memPointer, textureMemory[(size_t) textureMemoryIndex].imageLength, &imageWidth, &imageHeight, &numChannels, SOIL_LOAD_AUTO ))
	{
	...
	textureID = SOIL_load_OGL_texture_from_memory ((const unsigned char *) textureMemory[(size_t) textureMemoryIndex].memPointer, textureMemory[(size_t) textureMemoryIndex].imageLength, SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y ); //SOIL_FLAG_TEXTURE_REPEATS); SOIL_FLAG_MIPMAPS
	...
	evt_sendEvent ( USER_EVENT_TEXTURE, USER_EVENT_TEXTURE_UPLOAD_DONE, textureID, 0, 0, vec2{imageWidth,
	                                                                                          imageHeight}, vec2 (), textureMemory[(size_t) textureMemoryIndex].textureName );
...
//	unordered_map<string, _textureSet>::const_iterator textureItr;
	...
	auto textureItr = textureSet.find ( fileName );
...
//-----------------------------------------------------------------------------------------------------
{
	auto textureItr = textureSet.begin();

	while (textureItr != textureSet.end())
	{
		if (textureItr->second.markedForDeletion)
		{
			glDeleteTextures(1, &textureItr->second.textureID);
			textureItr = textureSet.erase(textureItr);

			printf("Deleted texture from map.\n");
		}
		else
			++textureItr;
	}
...
void io_removeTextureFromMap( const string textureName )
//-----------------------------------------------------------------------------------------------------
{
	auto textureItr = textureSet.find(textureName);

	if (textureItr != textureSet.end())
	{
		textureItr->second.markedForDeletion = true;        // Clean up in main thread that has the OpenGL context
