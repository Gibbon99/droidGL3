...
//-----------------------------------------------------------------------------
{
    if (keyAction == MY_INPUT_ACTION_RELEASE) {
        ...
        //
        // Use a lift
        if ((playerDroid.overTile == LIFT_TILE) && (playerDroid.velocity.x == 0.0f) &&
...
//					if ( true == playerWeaponReadyToFire )
                {
                    ...
                    evt_sendEvent(MAIN_LOOP_EVENT, MAIN_LOOP_EVENT_ADD_BULLET, dataBaseEntry[0].bulletType, -1, false,
                                  glm::vec2{
                                          playerDroid.middlePosition.x, playerDroid.middlePosition.y},
                                  glm::vec2{destPosition.x,
...
//						playerWeaponReadyToFire = false;
        ...
        if ((playerDroid.overTile != TERMINAL_TOP) && (playerDroid.overTile != TERMINAL_BOTTOM) &&
            (playerDroid.overTile != TERMINAL_LEFT) && (playerDroid.overTile != TERMINAL_RIGHT) &&
            (playerDroid.overTile != LIFT_TILE) && (playerDroid.velocity.x == 0.0f) &&
            (playerDroid.velocity.y == 0.0f))
        {
            printf("Enter transfer mode\n");
            playerDroid.currentMode = DROID_MODE_TRANSFER;
            evt_sendEvent (USER_EVENT_AUDIO, AUDIO_PLAY_SAMPLE, SND_TRANSFER_1, 0, -1, glm::vec2 (), glm::vec2 (), "");
            return;
        }
    }
}
...
//-----------------------------------------------------------------------------
{
    ...
    switch (currentMode) {
        ...
        case MODE_GAME:
            ...
            evt_sendEvent(USER_EVENT_GAME, USER_EVENT_KEY_EVENT, io_returnStandardInputActionKeyboard(event.type),
                          io_returnStandardInputValueKeyboard(event.key.keysym.sym), 0, glm::vec2{
...
//                net_sendCurrentLevel(lvl_getCurrentLevelName ());
        ...
        case MODE_DECK_VIEW:
        case MODE_DATABASE:
            evt_sendEvent(USER_EVENT_GUI, USER_EVENT_KEY_EVENT, io_returnStandardInputActionKeyboard(event.type),
                          io_returnStandardInputValueKeyboard(event.key.keysym.sym), false, glm::vec2{}, glm::vec2{},
                          "");
            break;
        ...
        case MODE_TRANSFER_FINISH:        // Clean up
            evt_sendEvent(USER_EVENT_TRANSFER, USER_EVENT_KEY_EVENT, io_returnStandardInputActionKeyboard(event.type),
                          io_returnStandardInputValueKeyboard(event.key.keysym.sym), false, glm::vec2{}, glm::vec2{},
                          "");
            break;
