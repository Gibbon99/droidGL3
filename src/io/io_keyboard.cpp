...
//				sys_pauseSoundSystem ( true );
      ...
      if (DROID_MODE_NORMAL == playerDroid.currentMode)
        {
		if ((playerDroid.velocity.x != 0.0f) || (playerDroid.velocity.y != 0.0f))
          {
//					if ( true == playerWeaponReadyToFire )
            {
              ...
              evt_sendEvent (MAIN_LOOP_EVENT, MAIN_LOOP_EVENT_ADD_BULLET, dataBaseEntry[0].bulletType, -1, false, glm::vec2{
                  playerDroid.middlePosition.x, playerDroid.middlePosition.y}, glm::vec2{destPosition.x,
                                                                                         destPosition.y}, lvl_getCurrentLevelName ());
//						playerWeaponReadyToFire = false;
  ...
  else
    {
      ...
      if ((playerDroid.overTile != TERMINAL_TOP) && (playerDroid.overTile != TERMINAL_BOTTOM) &&
          (playerDroid.overTile != TERMINAL_LEFT) && (playerDroid.overTile != TERMINAL_RIGHT) &&
          (playerDroid.overTile != LIFT_TILE) && (playerDroid.velocity.x == 0.0f) && (playerDroid.velocity.y == 0.0f))
        {
          printf ("Enter transfer mode\n");
            ...
            switch (eventData.data2)
              {
                ...
                case MY_INPUT_LEFT: eventMoveLeft = true;
                break;

                case MY_INPUT_RIGHT: eventMoveRight = true;
                break;

                case MY_INPUT_UP: eventMoveUp = true;
                break;

                case MY_INPUT_DOWN: eventMoveDown = true;
                break;

                case MY_INPUT_FORWARD: eventMoveForward = true;
                break;

                case MY_INPUT_BACKWARD: eventMoveBackward = true;
                break;

                case MY_INPUT_ACTION: io_processActionKey (MY_INPUT_ACTION_PRESS);
                break;
...
//				io_saveScreenToFile ();
                ...
                case MY_INPUT_PAUSE: con_print (CON_INFO, true, "Pressed the P key - action is DOWN");
                evt_sendEvent (USER_EVENT_MODE_PAUSE, 0, 0, 0, 0, glm::vec2 (), glm::vec2 (), "");
                break;
          ...
          switch (eventData.data2)
            {
              ...
              case MY_INPUT_LEFT: eventMoveLeft = false;
              break;

              case MY_INPUT_RIGHT: eventMoveRight = false;
              break;

              case MY_INPUT_UP: eventMoveUp = false;
              break;

              case MY_INPUT_DOWN: eventMoveDown = false;
              break;

              case MY_INPUT_FORWARD: eventMoveForward = false;
              break;

              case MY_INPUT_BACKWARD: eventMoveBackward = false;
              break;

              case MY_INPUT_ACTION: io_processActionKey (MY_INPUT_ACTION_RELEASE);
              break;
  ...
  if (SDL_KEYDOWN == action)
    {
      switch (key)
        {
          case SDLK_BACKQUOTE: sys_changeMode (-1);        // Back to previous mode
          conCurrentCharCount = 0;
          SDL_StopTextInput ();
          break;

          case SDLK_BACKSPACE: con_processBackspaceKey (0.0);
          break;

          case SDLK_RETURN: con_processCommand (conCurrentLine.conLine);
          break;

          case SDLK_TAB: con_completeCommand (conCurrentLine.conLine);
          break;
          ...
          case SDLK_UP: con_popHistoryCommand ();
          //
...
//--------------------------------------------------------------------------
{
  switch (eventType)
    {
      case SDL_KEYDOWN: return MY_INPUT_ACTION_PRESS;
      break;

      case SDL_KEYUP: return MY_INPUT_ACTION_RELEASE;
      break;
...
//--------------------------------------------------------------------------
{
  ...
  switch (keyValue)
    {
      case SDLK_BACKQUOTE: return MY_INPUT_CONSOLE;
      break;

      case SDLK_LEFT: return MY_INPUT_LEFT;
      break;

      case SDLK_RIGHT: return MY_INPUT_RIGHT;
      break;

      case SDLK_UP: return MY_INPUT_UP;
      break;

      case SDLK_DOWN: return MY_INPUT_DOWN;
      break;

      case SDLK_ESCAPE: return MY_INPUT_ESCAPE;
      break;

      case SDLK_LCTRL: return MY_INPUT_ACTION;
      break;

      case SDLK_RETURN: return MY_INPUT_ACTION;
      break;
...
//				io_saveScreenToFile ();
      ...
      case SDLK_PAUSE: return MY_INPUT_PAUSE;
      break;
...
//                net_sendCurrentLevel(lvl_getCurrentLevelName ());
      ...
      case MODE_SIDE_VIEW:
      case MODE_DECK_VIEW:
      case MODE_DATABASE: evt_sendEvent (USER_EVENT_GUI, USER_EVENT_KEY_EVENT, io_returnStandardInputActionKeyboard (event.type), io_returnStandardInputValueKeyboard (event.key.keysym.sym), false, glm::vec2{}, glm::vec2{}, "");
      break;

      case MODE_PAUSE: io_readPauseModeKey (event.key.keysym.sym, event.type);
      break;
